...
data_2008 = flights_tbl %>%
    ...
    left_join(airports_tbl, by = c("dest" = "faa")) %>% 
    filter(!is.na(tz_name) & !is.na(alt)) %>%
    select(year, month, arrdelay, depdelay, destination = dest, distance, uniquecarrier, description, tz_name, alt) %>% group_by(destination) %>% mutate(n_fromLAX = count()) %>% arrange(desc(n_fromLAX))

carrier <- sdf_predict(x = data_2008, model = ml1) %>%
