...
flightstotal = left_join(flightzorigin, flightzz, by = c("origin" = "dest")) %>% mutate(N = n_origin + n_dest) %>% arrange(desc(N))

names(flightstotal) = c("Airport", "n_origin", "n_dest", "Total Flights (N)")
...
ggplot() + 
      ...
      geom_point(data = top10busiestLatLong2, aes(x = Longitude, y = Latitude, size = `Total Flights (N)`), color = "black") +
      geom_point(data = top10busiestLatLong2, aes(x = Longitude, y = Latitude, size = `Total Flights (N)`), color = "chartreuse") +
      scale_size(name = "Total Flights (N)", range = c(1,5)) +
