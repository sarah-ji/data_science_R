...
x = "Longitude (Degrees)", y = "Latitude (Degrees)")  + theme(legend.position="none")
...
```{r}
#16
data08 = fromLAX3 %>% filter(year == 2008 & !is.na(tz_name) & !is.na(alt)) %>% select(uniquecarrier, tzname, arrdelay, alt, depdelay, distance)

carrier <- sdf_predict(x = data08, model = ml1) %>%
  group_by(uniquecarrier) %>% summarize(arrdelay = mean(arrdelay), prediction = mean(prediction), freq = n()) %>% filter(freq > 500) %>% collect()
...
```{r}
#17
data_post911 = fromLAX3 

pre911 = flights_tbl %>% filter(year <= 2001) %>%
    filter(!is.na(arrdelay) & !is.na(depdelay) & !is.na(distance) & origin == "LAX" & (cancelled == 0)) %>%
    filter(depdelay > 15 & depdelay < 240) %>% #filter outliers
    filter(arrdelay > -60 & arrdelay < 360) %>% #filter outliers
    left_join(airlines_tbl, by = c("uniquecarrier" = "code")) %>% # merge by the columns
    inner_join(airports_tbl, by = c("dest" = "faa")) %>% 
    filter(!is.na(tz_name) & !is.na(alt)) %>%
    select(year, month, arrdelay, depdelay, destination = dest, distance, uniquecarrier, description, tz_name, alt) %>% group_by(uniquecarrier) %>% summarise(arrivaldelay = mean(arrdelay)) %>% collect()

post911 = data_post911 %>% group_by(uniquecarrier) %>% summarise(arrivaldelay = mean(arrdelay)) %>% collect()

nineleven = inner_join(pre911, post911, by = "uniquecarrier")
names(nineleven) = c("uniquecarrier", "arrivaldelay", "arrivaldelay")

nineeleven <- rbind(
  data.frame(data = 'pre911', nineleven[,c(1,2)]),
  data.frame(data = 'post911', nineleven[,c(1,3)]),
  make.row.names = FALSE
)

nineeleven %>%
  ggplot(aes(factor(uniquecarrier), arrivaldelay, fill = data)) +
  geom_bar(stat = 'identity', position = 'dodge') +
  labs(title = 'Average Arrival Delay Pre and Post 9/11 (2001)', x = 'Unique Carrier', y = 'Minutes')

```

```{r}
plotn = data08 %>% group_by(tz_name) %>% summarize(arrdelay = mean(arrdelay), distance = mean(distance)) %>% filter(freq > 500) %>% collect()

  ggplot(plotn) + geom_boxploth(aes(x = tz_name, y = arrdelay,
