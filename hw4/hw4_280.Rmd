...
group_by(year, month, dayofmonth) %>% summarise(n1 = count()) %>% collect()

LAXwithseason = flights_tbl %>% filter((dest == "LAX" | origin = "LAX") & cancelled == 0 & !is.na(distance)) %>% 
  mutate(if(month %in% c(3:5)){season = "Spring"}
                      if(month %in% c(6:8)){season = "Summer"}
                      if(month %in% c(9:11)){season = "Autumn"}
                      if(month %in% c(12, 1, 2)){season = "Winter"} %>% group_by(season) %>% summarise(n_season = count()) %>% collect()


LAXseason <- LAXwithseason %>% group_by(season) %>% count() %>% collect()
```
...
LAXwithseason = flights_tbl %>% filter((dest == "LAX" | origin = "LAX") & cancelled == 0 & !is.na(distance)) %>% mutate( if(month %in% c(3:5)){season = "Spring"}
                      else if(month %in% c(6:8)){season = "Summer"}
                      else if(month %in% c(9:11)){season = "Autumn"}
                      else if(month %in% c(12, 1, 2)){season = "Winter"}) %>% collect()


LAXseason <- LAXwithseason %>% group_by(season) %>% count() %>% collect()

LAX2 <- flights_tbl %>%
  filter((dest == "LAX" | origin == "LAX") & (cancelled == 0) & !is.na(distance)) %>% 
  group_by(dayofweek) %>% summarise(n_weekday = n()) %>% arrange(desc(n_weekday)) %>% collect()
```
...
model_data <- flights_tbl %>%
    filter(!is.na(arrdelay) & !is.na(depdelay) & !is.na(distance) & (dest == "LAX" | origin == "LAX") & (cancelled == 0)) %>%
    ...
    select(year, month, arrdelay, depdelay, distance, uniquecarrier, description, gain) %>% collect()
...
model_partition <- model_data %>% 
  sdf_partition(train = 0.8, valid = 0.2, seed = 51738)
...
out <- flights_tbl %>%
  filter(dest == "LAX") %>% select(year, month, dayofmonth, dayofweek, uniquecarrier, airtime, arrdelay, depdelay, origin, dest, distance,cancelled,cancellationcode, diverted, carrierdelay, weatherdelay, nasdelay, securitydelay, lateaircraftdelay)
  group_by(year) %>%
  count() %>%
  arrange(year) %>%
  collect()
  
    my_model_data <- flights_tbl %>%
    filter(!is.na(arrdelay) & !is.na(depdelay) & !is.na(distance) & cancelled == 0) %>%
    filter(depdelay > 15 & depdelay < 240) %>% #filter outliers
    filter(arrdelay > -60 & arrdelay < 360) %>% #filter outliers
    filter(year >= 2003 & year <= 2007) %>%
    select(dayofweek, airtime, origin, dest, diverted, carrierdelay, weatherdelay, securitydelay) %>% collect()
