...
```{r}
  model_deciles <- lapply(model_partition, function(x) {
    sdf_predict(ml1, x) %>%
      ...
      group_by(decile) %>%
      summarize(arrivaldelay = mean(arrdelay)) %>%
      select(decile, arrivaldelay) %>%
      collect()
  })
```
...
carrier <- sdf_predict(ml1, data_2008) %>%
  ...
  summarize(arrival_delay = mean(arrdelay), prediction = mean(prediction), freq = n()) %>% 
