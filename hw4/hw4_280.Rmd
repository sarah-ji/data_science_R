...
```{r}
#not sure what to do about the seasonal effects

LAXflightsbyday[LAXflightsbyday$month %in% c(12, 1, 2), 5] = "Winter"
LAXflightsbyday[LAXflightsbyday$month %in% c(6:8), 5] = "Summer"
LAXflightsbyday[LAXflightsbyday$month %in% c(3:5), 5] = "Spring"
LAXflightsbyday[LAXflightsbyday$month %in% c(9:11), 5] = "Autumn"

LAXseason =  LAXflightsbyday %>% mutate(season = as.factor(V5)) %>% select(year, n, season)
 
LAXseason2 = LAXseason %>% group_by(season) %>% mutate(n_season = sum(n))
#springtest = LAXseason %>% filter(season == "Spring") %>% summarize(n = sum(n))

seasonal = LAXseason2[!duplicated(LAXseason2[5]),]
       
LAXseason2 %>% arrange(season) %>% ggplot() + geom_line(aes(x = season, y = n_season)) + labs(title = "LAX air traffic by season")
...
group_by(dayofweek) %>% summarise(n_week = count()) %>% collect()

LAXflightsbyweek %>% arrange(dayofweek) %>% ggplot() + geom_line(aes(x = dayofweek, y = n_week)) + labs(title = "LAX air traffic by week")
