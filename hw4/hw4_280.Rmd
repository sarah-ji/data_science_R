...
LAX = LAX %>% mutate( if(month %in% c(3:5)){season = "Spring"}
                      else if(month %in% c(6:8)){season = "Summer"}
                      else if(month %in% c(9:11)){season = "Autumn"}
                      else if(month %in% c(12, 1, 2)){season = "Winter"})


LAXdest <- flights_tbl %>% filter(dest == "LAX" & cancelled == 0 & !is.na(distance)) %>%
  group_by(year) %>% summarise(n_peryeardest = count()) %>% arrange(desc(year)) %>% collect()

LAX2 <- flights_tbl %>%
  filter((dest == "LAX" | origin == "LAX") & (cancelled == 0)) %>% select(year, month, dayofweek) %>%
