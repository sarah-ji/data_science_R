...
ggplot(data = diamonds, mapping = aes(x = carat)) +
  ...
  labs(title = "Distribution of Price using coord_cartesian() to zoom in on carat range 0 - 1")
```
...
flights %>% mutate(cancelled = is.na(dep_time) | is.na(arr_time)) %>%
    ...
    labs(title = "Departure Times for Cancelled (T) vs. Not Cancelled (F) Flights")
```
