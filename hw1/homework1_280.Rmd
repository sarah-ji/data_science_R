...
There are n = 959 people in the dataset. 

```{bash, echo= FALSE, eval = TRUE}
wc -l /home/m280-data/hw1/merge-geno.fam
```

There are 8,348,674 SNP's in the dataset.
...
#to output into a comma separated format we switch columns 2 and 1 and then skip 3 and print straight to 4 as mendel format requires
...
Yes I did. The finalized file in Mendel's pedigree formatting is stored in the 'SNPdeffile' file located on the teaching server.
...
```{r, echo= FALSE, eval = T}
library(knitr)
n100gaussian = read.table(file ="n100gaussian.txt")
...
dist_gaussian = rbind(n100gaussian, n200gaussian, n300gaussian, n400gaussian, n500gaussian)
