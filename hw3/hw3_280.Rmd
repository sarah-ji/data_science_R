...
tickets_per_year = LAtickets %>% group_by(Issue_Year) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_hour = LAtickets %>% group_by(Issue_Hour) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_weekday = LAtickets %>% group_by(Issue_Wday) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_month = LAtickets %>% group_by(Issue_Month) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_monthday = LAtickets %>% group_by(Issue_Day) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_make = LAtickets %>% group_by(Make) %>% summarize(n = n()) %>% arrange(desc(n))
...
tickets_per_color = LAtickets %>% group_by(Color) %>% summarize(n = n()) %>% arrange(desc(n))
...
```{r}
tickets_per_violationtype = LAtickets %>% group_by(Violation_Description) %>% summarize(n = n()) %>% arrange(desc(n))
...
```{r}
money_collected_156 = LAtickets %>% filter(Issue_Year == 2015 | Issue_Year == 2016) %>% group_by(Issue_Year) %>%
...
topticketedmakeswithothers = LAtickets %>% select(Make, Body_Style, Fine_amount) %>% group_by(Make) %>% summarise(n = n(), Body_Style = Body_Style, Fine_amount = Fine_amount) %>% arrange(desc(n))

top10ticketmakeswitho = collect(head(topticketedmakeswithothers, 10))

top10ticketmakeswitho %>% ggplot(mapping = aes(x = n)) +
