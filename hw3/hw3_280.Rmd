...
select(Year, `Department Title`, `Job Title`, `Projected Annual Salary`, `Total Payments`, `Base Pay`, `Overtime Pay`, `Other Pay (Payroll Explorer)`, `Average Health Cost`, `Hourly or Event Rate`, `Average Benefit Cost`, `Employment Type`)

names(LA_payroll) = c("Year", "Department_Title", "Job_Title", "Projected_Annual_Salary", "Total_Payments", "Base_Pay", "Overtime_Pay", "Other_Pay", "Average_Healthcare_Cost", "Hourly_Rate", "Average_Benefit_Cost", "Employment_Type")
...
**Answer**: In tab 1.3 of the shiny app, I have visualized the total LA City payroll by each year, with breakdown into base pay, overtime pay, other pay, Department title and Job Title of the top 10 highest paid LA city employees in a specified year, by default 2017.
...
**Answer**: In tab 1.3 of the shiny app, I visualize the mean or median payroll with breakdown into median base pay, median overtime pay, and median other pay, of top 5 earning departments in 2017, by default.  
...
ggplot(data = tickets_per_make, mapping = aes(x = n)) +
...
library(ggstance)
ggplot(tickets_per_make) + geom_boxploth(aes(x = Body_Style, y = cut_number(Fine_amount, 2), colour = Make))
